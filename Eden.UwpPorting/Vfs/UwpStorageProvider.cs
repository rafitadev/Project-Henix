...
public sealed class UwpStorageProvider : IVirtualFileSystem
{
    ...
    private static RuntimeImage ReadFromXci(Stream stream)
    {
        ...
        long dataOffset = 0x100 + 0x10 + fileCount * 0x18 + stringTableSize;
        ...
        foreach (var e in entries)
        {
            ...
            byte[] nspBlob = br.ReadBytes((int)e.Size);
            using var nspStream = new MemoryStream(nspBlob, writable: false);
    ...
    private readonly record struct PfsEntry(string Name, long Offset, long Size);
    ...
    private static RuntimeImage ReadFromPfs0(Stream stream)
    {
        ...
        var raw = new List<(long Off, long Size, int NameOff)>();
        for (int i = 0; i < fileCount; i++)
        {
            raw.Add((br.ReadInt64(), br.ReadInt64(), br.ReadInt32()));
            _ = br.ReadInt32();
        }

        byte[] strTab = br.ReadBytes(stringTableSize);
        long dataOff = 0x10 + fileCount * 0x18 + stringTableSize;
        var entries = raw.Select(r => new PfsEntry(ReadCString(strTab, r.NameOff), dataOff + r.Off, r.Size)).ToList();

        foreach (var e in entries)
        {
            byte[] blob = ReadBlob(stream, br, e.Offset, e.Size);
            if (e.Name.EndsWith(".nro", StringComparison.OrdinalIgnoreCase))
            {
                using var ms = new MemoryStream(blob, writable: false);
                return ReadNro(ms);
            }

            if (e.Name.EndsWith(".nso", StringComparison.OrdinalIgnoreCase) || e.Name.Equals("main", StringComparison.OrdinalIgnoreCase))
            {
                using var ms = new MemoryStream(blob, writable: false);
                return ReadNso(ms);
            }
        ...
        var cnmt = entries.FirstOrDefault(e => e.Name.EndsWith(".cnmt", StringComparison.OrdinalIgnoreCase) || e.Name.EndsWith(".cnmt.nca", StringComparison.OrdinalIgnoreCase));
        if (!string.IsNullOrEmpty(cnmt.Name))
        {
            byte[] cnmtBlob = ReadBlob(stream, br, cnmt.Offset, cnmt.Size);
            selectedProgram = TryFindProgramNcaNameFromCnmt(cnmtBlob, entries.Select(e => e.Name));
            if (selectedProgram is not null) BootDiagnostics.Info($"CNMT selecionou Program NCA: {selectedProgram}");
        }

        var ncas = entries.Where(e => e.Name.EndsWith(".nca", StringComparison.OrdinalIgnoreCase)).ToList();
        if (selectedProgram is not null)
        {
            ncas = ncas.OrderByDescending(e => e.Name.Equals(selectedProgram, StringComparison.OrdinalIgnoreCase)).ToList();
        }

        foreach (var nca in PrioritizeNcaCandidates(ncas.Select(e => (e.Name, ReadBlob(stream, br, e.Offset, e.Size))).ToList()))
        {
    ...
    private static byte[] ReadBlob(Stream stream, BinaryReader br, long offset, long size)
    {
        stream.Position = offset;
        return br.ReadBytes((int)size);
    }

    private static string? TryFindProgramNcaNameFromCnmt(byte[] cnmtBlob, IEnumerable<string> nspNames)
    {
        foreach (string name in nspNames)
        {
            if (!name.EndsWith(".nca", StringComparison.OrdinalIgnoreCase)) continue;
            string id = name[..^4];
            if (id.Length != 32) continue;
            byte[] idBytes = Convert.FromHexString(id);
            int idx = IndexOfBytes(cnmtBlob, idBytes);
            if (idx < 0) continue;
            int typePos = idx + 0x20;
            if (typePos < cnmtBlob.Length && cnmtBlob[typePos] == 0x1) return name;
        }
        return null;
    }
